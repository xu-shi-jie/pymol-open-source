...
int ExecutiveCheckGroupMembership(PyMOLGlobals* G, int list_id,
    pymol::CObject* obj); /* 0.5*N for group size */
...
int ExecutiveGroup(PyMOLGlobals* G, pymol::zstring_view name,
    pymol::zstring_view members, int action, int quiet);
...
pymol::Result<> ExecutiveOrder(
    PyMOLGlobals* G, pymol::zstring_view s1, int sort, int location);
...
pymol::Result<> ExecutiveLoad(PyMOLGlobals* G, const char* fname,
    ...
    const char* object_name, int state, int zoom, int discrete, int finish,
    int multiplex, int quiet, const char* plugin,
...
pymol::Result<> ExecutivePseudoatom(PyMOLGlobals* G,
    pymol::zstring_view object_name, const char* sele, const char* name,
...
pymol::Result<> ExecutiveLabel(PyMOLGlobals* G, const char* s1,
 ...
 */
template <typename T>
T* ExecutiveFindObject(PyMOLGlobals* G, pymol::zstring_view name)
{
...
int ExecutiveIterateObject(
    PyMOLGlobals* G, pymol::CObject** obj, void** hidden);
pymol::Result<std::vector<DiscardedRec>> ExecutiveDelete(
    PyMOLGlobals* G, pymol::zstring_view nameView, bool save = false);
 ...
 * @save if true, the rec's associated object/cgo will not be destroyed, and the
 * caller is responsible for repurging w/o saving.
 */
...
pymol::Result<> ExecutiveSetDihe(PyMOLGlobals* G, const char* s0,
    ...
    int quiet = 1);
int ExecutiveRMS(PyMOLGlobals* G, const char* sele1, const char* sele2,
    int mode, float refine, int max_cyc, int quiet, const char* oname,
    int state1, int state2, int ordered_selections, int matchmaker,
...
/**
 * Sets the rotation center for the scene or object around a selection's center
 * (or provided coords)
 * @param sele (null-safe) selection expression used to calculate rotation
 * center
...
int ExecutiveGetMoment(
    PyMOLGlobals* G, const char* name, double* mi, int state);
...
void ExecutiveSymExp(PyMOLGlobals* G, const char* name, const char* obj,
    const char* sele, float cutoff, int segi, int quiet);
...
pymol::Result<> ExecutiveRemoveAtoms(
    PyMOLGlobals* G, const char* s1, int quiet);
...
pymol::Result<std::vector<const char*>> ExecutiveGetNames(
    PyMOLGlobals*, int, int, const char*);
...
void ExecutiveMakeUnusedName(PyMOLGlobals* G, char* prefix, int length,
    bool alwaysnumber = true, int start = 1, const char* pattern = "%02d");
std::string ExecutiveGetUnusedName(
    PyMOLGlobals* G, const char* prefix = "tmp", bool alwaysnumber = true);
 ...
 * @return number of atoms selected in active selection and change in that
 * selected
 */
...
pymol::Result<> ExecutiveMapDouble(
    PyMOLGlobals* G, const char* name, int state);
...
int ExecutiveGetObjectMatrix(PyMOLGlobals* G, const char* name, int state,
...
pymol::Result<std::pair<float, float>> ExecutiveSpectrum(PyMOLGlobals* G,
    pymol::zstring_view s1, pymol::zstring_view expr, float min, float max,
    int first, int last, pymol::zstring_view prefix, int digits, int byres,
    int quiet);

pymol::Result<> ExecutiveReinitialize(
    PyMOLGlobals* G, int what, pymol::zstring_view pattern);
...
CField* ExecutiveGetVolumeField(
...
pymol::Result<std::vector<DiscardedRec>> ExecutiveSetName(PyMOLGlobals* G,
    pymol::zstring_view old_name, pymol::zstring_view new_name,
    bool save = false);
...
pymol::Result<> ExecutiveIsomeshEtc(PyMOLGlobals* G, const char* mesh_name,
    ...
    float carve, int map_state, int quiet, int mesh_mode, float alt_lvl);
...
int ExecutiveVolumeColor(
    PyMOLGlobals* G, const char* volume_name, float* colors, int ncolors);
...
char* ExecutiveGetObjectNames(PyMOLGlobals* G, int mode, const char* name,
    int enabled_only, int* numstrs);

CoordSet* ExecutiveGetCoordSet(PyMOLGlobals* G, const char* name, int state,
...
/**
 * Determines whether the given name is of the Executive type (Selection or
 * Object) provided
...
bool ExecutiveIsSpecRecType(
    PyMOLGlobals* G, pymol::zstring_view name, int execType);
...
std::vector<OrderRec> ExecutiveGetOrderOf(
    PyMOLGlobals* G, pymol::zstring_view nameList);
