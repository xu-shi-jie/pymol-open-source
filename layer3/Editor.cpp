    ...
    if ((sele1 >= 0) && (sele2 >= 0)) {
      ...
      if (obj1 && (obj1 == obj2)) {
        ...
        if ((at0 >= 0) && (at3 >= 0)) {
          ...
          SelectorCreateOrderedFromObjectIndices(
              G, cEditorDihe1, obj1, &at0, 1);
          SelectorCreateOrderedFromObjectIndices(
              G, cEditorDihe2, obj2, &at3, 1);
          ...
          ExecutiveSetSettingFromString(
              G, cSetting_float_labels, "1", cEditorDihedral, 0, true, true);
#ifndef _PYMOL_FREETYPE
          ExecutiveSetSettingFromString(
              G, cSetting_label_font_id, "4", cEditorDihedral, 0, true, true);
#else
          ExecutiveSetSettingFromString(
              G, cSetting_label_font_id, "8", cEditorDihedral, 0, true, true);
          ExecutiveSetSettingFromString(
              G, cSetting_label_size, "20", cEditorDihedral, 0, true, true);
#endif
          ExecutiveSetSettingFromString(G, cSetting_label_color, "brightorange",
              cEditorDihedral, 0, true, true);
        }
      }
    }
  }
...
static void EditorConfigMouse(PyMOLGlobals* G)
{
  ...
  if (mouse_mode && (!strcmp(mouse_mode, "3-Button Editing") ||
...

    {
      ...
      if ((action == cButModeMovFrag) || (action == cButModeMovObj) ||
    ...
    {
      int action = ButModeGet(G, button);
      if ((action == cButModeRotFrag) || (action == cButModeRotObj) ||
    ...
    {
      int action = ButModeGet(G, button);
      if ((action == cButModeMovFragZ) || (action == cButModeMovObjZ) ||
...
static int EditorGetEffectiveState(
    PyMOLGlobals* G, pymol::CObject* obj, int state)
{
  if (obj && (obj->type == cObjectMolecule)) {
    ObjectMolecule* objMol = (ObjectMolecule*) (void*) obj;
    if (!objMol)
      objMol = SelectorGetFastSingleObjectMolecule(
          G, SelectorIndexByName(G, cEditorSele1));
    if (!objMol)
      objMol = SelectorGetFastSingleObjectMolecule(
          G, SelectorIndexByName(G, cEditorSele2));
    if (!objMol)
      objMol = SelectorGetFastSingleObjectMolecule(
          G, SelectorIndexByName(G, cEditorSele3));
    if (!objMol)
      objMol = SelectorGetFastSingleObjectMolecule(
          G, SelectorIndexByName(G, cEditorSele4));

    if (objMol) {
      if ((objMol->NCSet == 1) && (state > 0))
        if (SettingGet_i(
                G, nullptr, objMol->Setting.get(), cSetting_static_singletons))
          return 0;
    }
  }
  return state;
...
int EditorDeselectIfSelected(
    PyMOLGlobals* G, ObjectMolecule* obj, int index, int update)
{
  CEditor* I = G->Editor;
  int result = false;
  int s, sele;
...
int EditorFromPyList(PyMOLGlobals* G, PyObject* list)
{
  ...
  if (!active_flag) {
    ...
    if (ok && (ll > 2))
      ok = PConvPyIntToInt(
          PyList_GetItem(list, 2), &bond_mode); /* newer session files */
    if (ok) {
...
int EditorLogState(PyMOLGlobals* G, int pkresi)
{
  ...
  if (SettingGetGlobal_i(G, cSetting_logging)) {
    ...
    OrthoLineType buffer, buf1 = "None", buf2 = "None", buf3 = "None",
                          buf4 = "None";
    int pkbond = 1;
    ...
    if (!EditorActive(G)) {
      ...
      ObjectMolecule *obj1 = nullptr, *obj2 = nullptr, *obj3 = nullptr,
                     *obj4 = nullptr;
      ...
      sprintf(buffer, "cmd.edit(%s,%s,%s,%s,pkresi=%d,pkbond=%d)", buf1, buf2,
          buf3, buf4, pkresi ? 1 : 0, pkbond ? 1 : 0);
        ...
        for (frg = 1; frg <= I->NFrag; frg++) {
          ...
          if (ObjectMoleculeIsAtomBondedToSele(obj0, i0, sele2) &&
            ...
            ObjectMoleculeTransformSelection(
                obj0, state, sele2, m, false, nullptr, false, false);
          }
        }
        if (found) {
          if (!quiet) {
      ...
      if (!((sele0 >= 0) && (sele1 >= 0) && (sele2 >= 0) && (obj0 == obj1))) {
        ...
        if ((i0 >= 0) && (i1 >= 0)) {
          ...
          if (vf1 && vf2) {
            ...
            ok = ObjectMoleculeTransformSelection(
                obj2, state, sele2, m, false, nullptr, false, false);
            SceneInvalidate(G);
...
int EditorIsAnActiveObject(PyMOLGlobals* G, ObjectMolecule* obj)
{
  if (EditorActive(G)) {
    if (obj) {
      if (obj == SelectorGetFastSingleObjectMolecule(
                     G, SelectorIndexByName(G, cEditorSele1)))
        return true;
      if (obj == SelectorGetFastSingleObjectMolecule(
                     G, SelectorIndexByName(G, cEditorSele2)))
      ...
      if (obj0) {
        if (obj0->DiscreteFlag) {
          ...
          ObjectMoleculeVerifyChemistry(
              obj0, -1); /* remember chemistry for later */
          if (obj1) {
            if (obj0 == obj1) {
              ...
              return pymol::make_error(
                  "Picked atoms must belong to the same object.");
            }
          } else {
...
#define cEditorRemoveSele "_EditorRemove"
  ...
  if ((sele1 >= 0) && (obj0 == obj1) && I->BondMode) {
    ...
    if (SelectorIndexByName(G, cEditorSet) < 0) {
      ...
      if (hydrogen) {
        auto buf =
            pymol::string_format("((neighbor %s) and hydro)", cEditorSele1);
        h_flag = SelectorCreate(
            G, cEditorRemoveSele, buf.c_str(), nullptr, false, nullptr)
                     .result();
      }
      ...
      if (hydrogen) {
        auto buf =
            pymol::string_format("((neighbor %s) and hydro)", cEditorSet);
        h_flag = SelectorCreate(
            G, cEditorRemoveSele, buf.c_str(), nullptr, false, nullptr)
                     .result();
      }
...
pymol::Result<> EditorHFix(PyMOLGlobals* G, const char* sele, int quiet)
{
  ...
  ObjectMolecule *obj0, *obj1;
  if ((!sele) ||
      (!sele[0])) { /* if selection is empty, then apply to picked atoms */
    if (EditorActive(G)) {
      sele0 = SelectorIndexByName(G, cEditorSele1);
      if (sele0 >= 0) {
        obj0 = SelectorGetFastSingleObjectMolecule(G, sele0);
  ...
  if (EditorActive(G)) {
    ...
    if (G->HaveGUI && G->ValidContext) {
      ...
      if ((sele1 >= 0) && (sele2 >= 0) && I->BondMode && obj1 && obj2) {
        ...
        if (obj1) {
          ...
          if ((frozen = SettingGetIfDefined_i(
                   obj1->G, obj1->Setting.get(), cSetting_state, &st))) {
            state = st - 1;
          }
        ...
        if (obj2) {
          if ((frozen = SettingGetIfDefined_i(
                   obj2->G, obj2->Setting.get(), cSetting_state, &st))) {
            state = st - 1;
          }
        ...
        if (obj3) {
          if ((frozen = SettingGetIfDefined_i(
                   obj3->G, obj3->Setting.get(), cSetting_state, &st))) {
            state = st - 1;
          }
        ...
        if (obj4) {
          if ((frozen = SettingGetIfDefined_i(
                   obj4->G, obj4->Setting.get(), cSetting_state, &st))) {
            state = st - 1;
          }
    ...
    if (shaderCGO) {
      ...
      if (ok) {
        ...
        if (ok)
          ok &= CGODisable(tmpCGO, GL_TWO_SIDED_LIGHTING);
 ...
 */
static void EditorAutoMeasure(
    PyMOLGlobals* G, int sele1, int sele2, int sele3, int sele4, int state)
{
  if (sele1 < 0 || sele2 < 0)
    return;

  if (sele3 < 0) {
    ExecutiveDistance(G, cEditorMeasure, cEditorSele1, cEditorSele2, 0, -1.f,
        true, true, false /* reset */, state, false);
  } else if (sele4 < 0) {
...
void EditorActivate(PyMOLGlobals* G, int state, int enable_bond)
{
  ...
  if ((sele1 >= 0) || (sele2 >= 0) || (sele3 >= 0) || (sele4 >= 0)) {
    ...
    I->NFrag = SelectorSubdivide(G, cEditorFragPref, sele1, sele2, sele3, sele4,
...
/*========================================================================*/
void EditorSetDrag(
    PyMOLGlobals* G, pymol::CObject* obj, int sele, int quiet, int state)
{
  EditorInactivate(G);
...
/*========================================================================*/
void EditorPrepareDrag(PyMOLGlobals* G, pymol::CObject* obj, int sele,
    int index, int state, int mode)
{
  int frg;
  ...
  if ((!EditorActive(G)) || (!objMol)) {
    ...
    if (seleFlag) { /* normal selection */
      ...
      sprintf(name, "%s%1d", cEditorBasePref,
          frg); /* get relevant base vertex of bond */
  ...
  "BondFlag %d SlowFlag %d seleFlag %d\n",
      I->DragIndex, I->DragSelection, (void*) I->DragObject, I->DragHaveAxis,
      I->DragHaveBase, I->DragBondFlag, I->DragSlowFlag, seleFlag ENDFD;
}
...
void EditorDrag(PyMOLGlobals* G, pymol::CObject* obj, int index, int mode,
    int state, float* pt, float* mov, float* z_dir)
{
  CEditor* I = G->Editor;
  ...
  "Sele %d Object %p\n Axis %d Base %d BondFlag %d SlowFlag %d\n",
      (void*) obj, state, index, mode, I->DragIndex, I->DragSelection,
      (void*) I->DragObject, I->DragHaveAxis, I->DragHaveBase, I->DragBondFlag,
      I->DragSlowFlag ENDFD;
  ...
  if (obj) {
    ...
    if ((index == I->DragIndex) && (obj == I->DragObject)) {
      if (!EditorActive(G)) {
        int matrix_mode = SettingGet_i(
            G, I->DragObject->Setting.get(), nullptr, cSetting_matrix_mode);
        if (matrix_mode < 0)
        ...
        case cButModeRotDrag:
          ...
          if (matrix_mode && (I->DragSelection < 0)) {
            ...
            case 2:
              if (objMol)
                ObjectMoleculeTransformState44f(
                    objMol, state, m, log_trans, false, true);
              break;
            }
          } else {
            if (objMol)
        ...
        case cButModeRotView:
          ...
          if (mode == cButModeRotView) {
            ...
            if (matrix_mode) {
              ...
              if (objMol)
                ObjectMoleculeTransformSelection(objMol, state,
                    I->DragSelection, m, log_trans, I->DragSeleName, false,
                    true);
            }
          }
          SceneInvalidate(G);
        ...
        case cButModeMovViewZ:
          ObjectTranslateTTT(
              obj, mov, SettingGetGlobal_b(G, cSetting_movie_auto_store));
          break;
        case cButModeMovObj:
        ...
        case cButModeMovDragZ:
          if (matrix_mode && (I->DragSelection < 0)) {
            ...
            case 2:
              if (objMol)
                ObjectMoleculeTransformState44f(
                    objMol, state, m, log_trans, true, true);
              break;
            }
          } else {
            identity44f(m);
        ...
        case cButModePkTorBnd:
          if (I->DragHaveAxis) {
            ...
            project3f(d0, n0, v2);
            add3f(I->Center, v2,
                v2); /* v2 is the perpendicular point on the axis */
            subtract3f(pt, v2, d2);
