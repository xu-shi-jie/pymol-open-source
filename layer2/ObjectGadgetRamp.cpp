...
static void ObjectGadgetRampCalculate(
    ObjectGadgetRamp* I, float v, float* result)
{
  const float _1 = 1.0F;
  const float _0 = 0.0F;
  /* from Filipe Maia */
...
static int _ObjectGadgetRampInterpolate(
    ObjectGadgetRamp* I, float level, float* color, const float* table)
{
    ...
    if (level_is_ge != level_is_le) {
      if (level_is_le == 0) { /* lower extreme */
        ...
        if (fabs(d) > R_SMALL8) {
          ...
          x1 = 1.0F - x0;
          for (i = 0; i < 3; i++) {
            color[i] = x0 * table[3 * level_is_ge + i] +
                       x1 * table[3 * level_is_le + i];
          }
          clamp3f(color);
        } else {
...
static int _ObjectGadgetRampBlend(
    ObjectGadgetRamp* I, float* color, const float* table, int mode)
{
  ...
  case 3:
    ...
    if (i_level && table) {
      ...
      for (i = 0; i < n_level; i++) {
        for (j = 0; j < 3; j++) {
          color[j] =
              (color[j] < table[3 * i + j]) ? color[j] : table[3 * i + j];
        }
      }
      clamp3f(color);
    }
...
static int ObjectGadgetRampInterpolateWithSpecial(ObjectGadgetRamp* I,
    float level, float* color, const float* atomic, const float* object,
...
PyObject* ObjectGadgetRampAsPyList(ObjectGadgetRamp* I)
{
  ...
  bool any = false;
  int* special = nullptr;
  int pse_export_version =
      SettingGetGlobal_f(I->G, cSetting_pse_export_version) * 1000;
  ...
  PyList_SetItem(
      result, 9, any ? PConvIntVLAToPyList(special) : PConvAutoNone(nullptr));
  VLAFreeP(special);

  PyList_SetItem(result, 10,
      PConvAutoNone(nullptr) /* I->Extreme, removed in PyMOL 1.8 */);
    ...
    PyMOLGlobals* G, PyObject* list, ObjectGadgetRamp** result, int version)
{
  ...
  if (ok)
    ok = CPythonVal_PConvPyStrToStr_From_List(
        G, list, 6, I->SrcName, WordLength);
  if (ok)
    ...
    if (!ExecutiveValidateObjectPtr(I->G, I->Mol, cObjectMolecule))
      ...
      if (ok) {
        ...
        if (SettingGet_b(I->G, I->Setting.get(), nullptr,
          ...
          float atomic[3];
          int index = ObjectMoleculeGetNearestBlendedColor(
              I->Mol, pos, cutoff, state, &dist, atomic, sub_vdw);
          if (index >= 0) {
            const float* object = ColorGetRaw(I->G, I->Mol->Color);

            if (!ObjectGadgetRampInterpolateWithSpecial(
                    I, dist, color, atomic, object, pos, state, false)) {
              copy3f(I->Color.data(), color);
            }
          } else {
            float white[3] = {1.0F, 1.0F, 1.0F};
            if (!ObjectGadgetRampInterpolateWithSpecial(
                    I, cutoff + 1.0F, color, white, white, pos, state, false)) {
              copy3f(I->Color.data(), color);
            }
          }
        } else {
          int index = ObjectMoleculeGetNearestAtomIndex(
              I->Mol, pos, cutoff, state, &dist);
          if (index >= 0) {
            const float* atomic =
                ColorGetRaw(I->G, I->Mol->AtomInfo[index].color);
            const float* object = ColorGetRaw(I->G, I->Mol->Color);
...
static void ObjectGadgetRampUpdateCGO(ObjectGadgetRamp* I, GadgetSet* gs)
{
  ...
  CGOVertex(
      cgo, I->width + I->border, -(I->border + I->bar_height), I->border); // 10
  CGOVertex(
      cgo, I->width + I->border, -(I->height + I->border), I->border); // 8
  CGOEnd(cgo);

  CGOColor(cgo, 1.0F, 1.0F, 1.0F);
  CGONormal(cgo, 0.f, 0.f,
      1.f); // text/characters need z-normal to include normals in vbo
  if (I->Level && I->NLevel) {
    float exindent = (n_color > 0) ? I->bar_height : 0.f;
    float pos[] = {I->border + I->text_border + exindent,
        I->text_border - (I->border + I->height), I->border + I->text_raise};
  ...
  if (n_color > 0) {
    ...
    for (a = -1; a <= n_color; a++) {
      ...
      if (a == n_color) {
        ...
        v1 += I->bar_height + (I->width - 2 * I->bar_height) *
                                  (i_level[a] - i_level[0]) / range;
        if (a != n_color - 1) {
          src += 3;
        }
      }
    ...
    for (a = 0; a < samples; a++) {
      ...
      CGOVertex(cgo, I->border + (I->width * fxn), -(I->border + I->bar_height),
          I->border);
    }
  }
  /* center */
  ...
  CGOVertex(
      cgo, I->width + I->border * 2, -(I->height + I->border * 2), 0.f); // 4
  CGONormal(cgo, 0.f, 0.f, 1.f);                                  // normal 2
  CGOVertex(cgo, I->border, -(I->height + I->border), I->border); // 7
  CGOVertex(
      cgo, I->width + I->border, -(I->height + I->border), I->border); // 8
...
static int ObjectGadgetRampHandleInputColors(ObjectGadgetRamp* I)
{
  ...
  if (I->Color) {
    ...
    if (n_color != I->NLevel && I->NLevel != 2) {
      ...
      " GadgetRamp-Warning: number of colors (%d) and number of levels (%d) "
      "don't\n"
      " match and n_level != 2. Support for trailing extreme colors dropped in "
      "1.8.",
    ...
    float within, float sigma, int zero, int calc_mode)
{
...

  {
    ...
    if (vert_vla && map && (ms = ObjectMapGetState(map, map_state))) {
      if (ObjectMapStateGetExcludedStats(
              G, ms, vert_vla, beyond, within, tmp_level)) {
...
void ObjectGadgetRamp::invalidate(cRep_t rep, cRepInv_t level, int state) {}

/*========================================================================*/
ObjectGadgetRamp::ObjectGadgetRamp(PyMOLGlobals* G)
    : ObjectGadget(G)
{
  auto I = this;
