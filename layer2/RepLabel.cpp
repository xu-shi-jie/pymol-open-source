...
struct VItemType {
  ...
  float screen_point[3];    //  6- 8: coordinate + label_placement_offset or
                            //  label_screen_point
   ...
   */
  int getDrawVar() const noexcept {
    constexpr int DRAW_VAR_MASK = 0b01111111;
    return DRAW_VAR_MASK & static_cast<int>(draw_var_f);
  }
};
...
short InvalidateShaderCGOIfTextureNeedsUpdate(
    PyMOLGlobals* G, float font_size, int texture_font_size, int* sizeArg)
{
  ...
  if (size <= 0)
    size = 1;
  if (size > MAX_LABEL_TEXTURE_SIZE) { // if label size is above max, just set
                                       // to max size
    ...
    if (font_size > 0 || size < MAX_LABEL_FOR_ALWAYS_REFRESH) {
      ...
      // label is larger than min, but changes more than a percentage, refresh
      // on texture
...
static void RepLabelAdjustScreenZ(PyMOLGlobals* G, float* pt)
{
  ...
  if (!outside) {
    pt[2] = CLAMP_VALUE(
        pt[2], -.9999, .97); // .98 b/c resolution in the back is lower
  }
}

static void addXYtoVertex(
    float x, float y, float* xv, float* yv, float* origv, float* v)
{
  float tmp3f[3];
  copy3f(origv, v);
  mult3f(xv, x, tmp3f);
    ...
    unsigned char noLighting)
{
  ...
  addXYtoVertex(-dirv[0], -dirv[1], xn, yn, pt1, pt1E);
  addXYtoVertex(
      bottomext * linev[0], bottomext * linev[1], xn, yn, pt1E, pts[2]);

  addXYtoVertex(-dirv[0], -dirv[1], xn, yn, pt2, pt2E);
  addXYtoVertex(
      -bottomext * linev[0], -bottomext * linev[1], xn, yn, pt2E, pts[3]);
...
static void drawLine2DCross(
    float cw, float x1, float y1, float x2, float y2, float* cross)
{
  float lvect[3];
  float nzn[3] = {0.f, 0.f, 1.f};
...
/* Draw Line/Polygon from point (x1,y1) to (x2,y2) with different Z's, where the
   current Z is at world point curpt with the line starting at (x1,y1), and the
   second point (x2,y2) is at the world point pt (i.e., the offset is embedded
   into the matrix convMatrix computed from SceneGenerateMatrixToAnotherZFromZ
 */
static void drawLineToPointInWorldCross(PyMOLGlobals* G, float cw, float x1,
    float y1, float x2, float y2, float* cross, float* pt, float* curpt)
{
  float lvect[3];
  float nzn[3] = {0.f, 0.f, 1.f};
  float convMatrix[16];
...
static short CLIPt(float denom, float num, float* tE, float* tL,
...
/* This function clips the line inside of the (-xmax,-ymax, xmax, ymax)
 * rectangle */
static void Clip2D(float xmax, float ymax, float* x0, float* y0, float* x1,
    float* y1, short* visible, short* clipedges)
{
  float dx = *x1 - *x0;
  float dy = *y1 - *y0;
  *visible = 0;
...
static void Clip2DLine(
    float xmax, float ymax, float* line, short* visible, short* clipedges)
{
  Clip2D(
      xmax, ymax, &line[0], &line[1], &line[2], &line[3], visible, clipedges);
}

static void glVertex3fTransformed(float* convMatrix, float x, float y, float z)
{
  float tmppt[3] = {x, y, z};
  MatrixTransformC44f3f(convMatrix, tmppt, tmppt);
  glVertex3fv(tmppt);
...
static void drawLineToPointInWorldCrossClip(PyMOLGlobals* G, int label_z_target,
    float cw, float x1, float y1, float x2, float y2, float* cross, float* pt,
    float* curpt, float cx, float cy)
{
  float lvect[3];
  float nzn[3] = {0.f, 0.f, 1.f};
  float convMatrix[16];
...
static void drawLine2DCheckZTargetCross(PyMOLGlobals* G, short label_z_target,
    float* pt, float* curpt, float cw, float x1, float y1, float x2, float y2,
    float* cross)
{
  if (label_z_target) {
...
static void drawLine2DCheckZTarget(PyMOLGlobals* G, short label_z_target,
    float* pt, float* curpt, float cw, float x1, float y1, float x2, float y2)
{
  float cross[3];
  drawLine2DCheckZTargetCross(
      G, label_z_target, pt, curpt, cw, x1, y1, x2, y2, cross);
}

static void drawLine2DCheckZTargetClip(PyMOLGlobals* G, short label_z_target,
    float* pt, float* curpt, float cw, float x1, float y1, float x2, float y2,
    float cx, float cy)
{
  float cross[3];
  drawLineToPointInWorldCrossClip(
      G, label_z_target, cw, x1, y1, x2, y2, cross, pt, curpt, cx, cy);
}

void drawLineAsGeometryWithOffsets(float* pt1, float* pt2, float* spt1,
    float* spt2, float* xn, float* yn, float* zn, float line_width,
    float topext, float bottomext, float* dirv)
{
  float pt1E[3], pt2E[3];
  float tmpV[3], tmpV2[3], linev[3];
  float nzn[3] = {0.f, 0.f, 1.f};
  ...
  addXYtoVertex(-dirv[0], -dirv[1], xn, yn, pt2, pt2E);
  addXYtoVertex(
      -bottomext * linev[0], -bottomext * linev[1], xn, yn, pt2E, pt2E);
  glVertex3fv(pt2E);

  glEnd();
}

static void RepLabelRenderBackgroundInImmediate(PyMOLGlobals* G, RepLabel* I,
    float* v, int draw_var, float* tCenterPt, short relativeMode, float* xn,
    float* yn, float* PmvMatrix, float* RotMatrix, int screenwidth,
    int screenheight, float* screenWorldOffset, float* indentFactor,
    float text_width, float text_height, float font_size)
{
  ...
  short label_connector_mode = (draw_var & 8)    ? 1
                               : (draw_var & 16) ? 2
                               : (draw_var & 32) ? 3
                               : (draw_var & 64) ? 4
                                                 : 0;
  ...
  float indentFactorT[2] = {
      indentFactor[0] * text_width / 2.f, indentFactor[1] * text_height / 2.f};

  { // taking into account screen adjustments : indent and screen world offset
    // to the label point
    float v_scale, xn[3], yn[3];
    v_scale = SceneGetScreenVertexScale(G, nullptr);
    SceneGetScaledAxesAtPoint(G, labelpos, xn, yn);
    addXYtoVertex(indentFactorT[0] + screenWorldOffset[0] / v_scale,
        indentFactorT[1] + screenWorldOffset[1] / v_scale, xn, yn, labelpos,
        pos);
  }

  {
    ...
    normalize4f(tTarget);
    if (!((relativeMode & 2) ||
            (relativeMode & 4))) { // label_relative_mode = 0, not 1 or 2
    ...
    dVectorInPixels[0] = .5f * (screenwidth * tVec[0]) -
                         (screenWorldOffset[0] / v_scale) - indentFactorT[0];
    dVectorInPixels[1] = .5f * (screenheight * tVec[1]) -
                         (screenWorldOffset[1] / v_scale) - indentFactorT[1];
    dVector[0] = (dVectorInPixels[0] / text_width);
    dVector[1] = (dVectorInPixels[1] / text_height);
    doNotDraw =
        ((fabs(dVector[0])) <= .5f && (fabs(dVector[1])) <= .5f) ? 1.f : 0.f;
    ...
    if (drawLine) {
      ...
      case 0: {
        ...
        float right =
            (1.f - hmid) * ((dVector[0] > 0.f) ? 1.f : 0.f) + hmid * .5f;
        float top =
            (1.f - vmid) * ((dVector[1] > 0.f) ? 1.f : 0.f) + vmid * .5f;
        float labx, laby;
        xoff = 2.f * (right - .5);
        yoff = 2.f * (top - .5);
        labx = xoff * hwidth;
      ...
      case 3: {
        ...
        float right = (1.f - hmid) * ((dVector[0] > 0.f) ? 1.f : 0.f) +
                      hmid * (.5f + dVector[0]);
        float top = (1.f - vmid) * ((dVector[1] > 0.f) ? 1.f : 0.f) +
                    vmid * (.5f + dVector[1]);
        float labx, laby;
        xoff = 2.f * (right - .5);
        yoff = 2.f * (top - .5);
      ...
      case 1: {
        ...
        if ((draw_var & 6) == 2) {
          ...
          drawLine2DCheckZTargetClip(G, label_z_target, v + 3, pos, cw, 0, 0,
              dVectorInPixels[0], dVectorInPixels[1], hwidth, hheight);
        } else {
          drawLine2DCheckZTarget(G, label_z_target, v + 3, pos, cw, labx, laby,
              dVectorInPixels[0], dVectorInPixels[1]);
        }
      ...
      case 4: {
        ...
        if (hmid) {
          ...
          right =
              rightorig +
              (rightorig - .5f) *
                  CLAMP_VALUE(fabs(2.f * dVector[0]) - 1.f, 0.f,
        ...
        { // drawing the triangle filler between the two lines (connector_mode 2
          // or 4)
          ...
          dirv1[0] = -dirv1[0];
          dirv1[1] = -dirv1[1]; // switch normal b/c drawLine2DCheckZTargetCross
                                // call above has target as second
          dirv1[2] = dirv2[2] = 0.f;
...
static int lineSegIntersection(
    float* p1, float* p2, float* p3, float* p4, float* po)
{
  ...
  p4i[0] = newX;
  if ((p3i[1] < 0. && p4i[1] < 0.) || (p3i[1] >= 0. && p4i[1] >= 0.))
    return 0;
...
static void RepLabelRenderRayBackground(
    RepLabel* I, RenderInfo* info, float* v, int draw_var)
{
  CRay* ray = info->ray;
  PyMOLGlobals* G = I->G;
  ...
  mult3f(xn,
      indentFactor[0] * text_width + 2.f * (screenWorldOffset[0] / v_scale),
      tmp3f);
  add3f(tmp3f, torigCenter, torigCenter);
  mult3f(yn,
      indentFactor[1] * text_height + 2.f * (screenWorldOffset[1] / v_scale),
      tmp3f);
  add3f(tmp3f, torigCenter, torigCenter);

  mult3f(zn, -1.f,
      tmp3f); // push background back, not sure if this is needed, doesn't hurt
  add3f(tmp3f, torigCenter, torigCenter);

  if (draw_var & 6) { // draw background or background outline
    float tmpf[4][4];
    float hwidth = ray->Sampling * text_width / 2.f,
          hheight = ray->Sampling * text_height / 2.f;
    ...
    if (draw_var & 2) { // draw background
      ...
      ray->triangleTrans3fv(tmpf2[0], tmpf2[1], tmpf2[2], zn, zn, zn, &v[23],
          &v[23], &v[23], trans, trans, trans);
      ray->setLastToNoLighting(1);
      ray->triangleTrans3fv(tmpf2[1], tmpf2[2], tmpf2[3], zn, zn, zn, &v[23],
          &v[23], &v[23], trans, trans, trans);
      ray->setLastToNoLighting(1);
    }
    if (draw_var & 4) { // draw background outline
      if (label_con_flat) {
        ...
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[2], tmpf[0], tmpfs[2],
            tmpfs[0], xn, yn, zn, lw, 1.f, 0.f, v + 9, dirv, 1);
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[1], tmpf[0], tmpfs[1],
            tmpfs[0], xn, yn, zn, lw, 0.f, 1.f, v + 9, dirv, 1);
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[3], tmpf[1], tmpfs[3],
            tmpfs[1], xn, yn, zn, lw, 0.f, 1.f, v + 9, dirv, 1);
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[3], tmpf[2], tmpfs[3],
            tmpfs[2], xn, yn, zn, lw, 1.f, 0.f, v + 9, dirv, 1);
      } else {
  ...
  case 4:
    if (drawLine) {
      ...
      if (hmid) {
        ...
        rightorig = dVector[0] >= 0.f ? 1.f : 0.f;
        right = rightorig + (rightorig - .5f) *
                                CLAMP_VALUE(fabs(dVector[0] * 2.f) - 1.f, 0.f,
                                    2.f * extLength *
                                        fabs(text_height / (float) text_width));
      }
      if (vmid) {
        top = ((1.f + dVector[1]) / 2.f);
      } else {
      ...
      if (label_con_flat) {
        ...
        MatrixTransformC44f4f(ray->ModelView, tmpf[1], tmpfs[1]);
        MatrixTransformC44f4f(ray->ModelView, tmpf[2], tmpfs[2]);
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[0], tmpf[1], tmpfs[0],
            tmpfs[1], xn, yn, zn, connector_width, 0.f, 0.f, v + 9, dirv1, 1);
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[1], tmpf[2], tmpfs[1],
            tmpfs[2], xn, yn, zn, connector_width, 0.f, 0.f, v + 9, dirv2, 1);
        ...
        ray->sausage3fv(
            endpointExtendedOffBBX, endpointOnBBX, lw, v + 9, v + 9);
      }
      drawLine = 0;
    }
    break;
  ...
  if (drawLine) {
    ...
    if (label_con_flat) {
      ...
      if ((draw_var & 2) && (label_connector_mode > 0)) { // draw background
        ...
        if (lineSegIntersection(
                labelTop.p1, labelTop.p2, l1.p1, l1.p2, isec1)) {
          l1V = 1;
        } else if (lineSegIntersection(
                       labelRight.p1, labelRight.p2, l1.p1, l1.p2, isec1)) {
          l1V = 2;
        } else if (lineSegIntersection(
                       labelBottom.p1, labelBottom.p2, l1.p1, l1.p2, isec1)) {
          l1V = 3;
        } else if (lineSegIntersection(
                       labelLeft.p1, labelLeft.p2, l1.p1, l1.p2, isec1)) {
          l1V = 4;
        }

        if (lineSegIntersection(
                labelTop.p1, labelTop.p2, l2.p1, l2.p2, isec2)) {
          l2V = 1;
        } else if (lineSegIntersection(
                       labelRight.p1, labelRight.p2, l2.p1, l2.p2, isec2)) {
          l2V = 2;
        } else if (lineSegIntersection(
                       labelBottom.p1, labelBottom.p2, l2.p1, l2.p2, isec2)) {
          l2V = 3;
        } else if (lineSegIntersection(
                       labelLeft.p1, labelLeft.p2, l2.p1, l2.p2, isec2)) {
          l2V = 4;
        }
        ...
        if ((l1V && l2V)) {
          ...
          if (l1V == l2V) {
            ...
            ray->triangle3fv(
                pt1E, pt2E, isec1w, zn, zn, zn, v + 9, v + 9, v + 9);
            ray->triangle3fv(
                pt2E, isec1w, isec2w, zn, zn, zn, v + 9, v + 9, v + 9);
          } else { // (l1V != l2V)
            float isecc[3];
            ...
            ray->triangle3fv(
                isec1w, pt1E, iseccw, zn, zn, zn, v + 9, v + 9, v + 9);
            ray->triangle3fv(
                pt1E, iseccw, pt2E, zn, zn, zn, v + 9, v + 9, v + 9);
            ray->triangle3fv(
                iseccw, pt2E, isec2w, zn, zn, zn, v + 9, v + 9, v + 9);
          }
        }
      }
      if (drawDefaultLine) {
        ...
        MatrixTransformC44f4f(ray->ModelView, tmpf[1], tmpfs[1]);
        RayDrawLineAsGeometryWithOffsets(ray, tmpf[0], tmpf[1], tmpfs[0],
            tmpfs[1], xn, yn, zn, connector_width, 0.f, 0.f, v + 9, dirv, 1);
      }
    } else {
      ray->sausage3fv(v + 3, endpointOnBBX,
          connector_width * ray->PixelRadius / 2.f, v + 9, v + 9);
    }
  }
}

static void RepLabelRenderRay(RepLabel* I, RenderInfo* info)
{
...
void RepLabelPick(RepLabel* I, RenderInfo* info)
{
  auto* G = I->G;
  auto* pick = info->pick;
  int pick_labels = SettingGet_b(
    G, I->cs->Setting.get(), I->obj->Setting.get(), cSetting_pick_labels);
...
void RepLabelRenderImmediate(RepLabel* I, RenderInfo* info)
{
  auto G = I->G;
  auto font_id = SettingGet_i(
      G, I->cs->Setting.get(), I->obj->Setting.get(), cSetting_label_font_id);
  auto font_size = SettingGet_f(
      G, I->cs->Setting.get(), I->obj->Setting.get(), cSetting_label_size);
  auto float_text = SettingGet<bool>(
      G, I->cs->Setting.get(), I->obj->Setting.get(), cSetting_float_labels);
...
#endif
  TextSetOutlineColor(G, I->OutlineColor);
  auto* PmvMatrix = SceneGetPmvMatrix(G);
  int screenwidth, screenheight;
  SceneGetWidthHeight(G, &screenwidth, &screenheight);
  ...
  for (int c = 0; c < I->N; c++) {
    ...
    if (l) {
      ...
      if (relativeMode & 8) { // label_z_target, adjust z to target
        TextGetLabelPos(G)[0] =
            (SceneGetDepth(G, I->labelV[c].coord) - .5) * 2.f;
        TextSetLabelPosIsSet(G, 1);
      } else if (relativeMode & 6) { // label_relative_mode 1 or 2, i.e.,
                                     // screen stabilized, adjust z
      ...
      if (!TextRenderOpenGL(G, info, font_id, st, font_size,
              ...
              I->labelV[c].getRelativeMode(), false, I->shaderCGO)) {
      ...
      if (draw_var) {
        ...
        float* screenWorldOffset = TextGetScreenWorldOffset(G);
        float text_width = TextGetWidth(G);
        float text_height = TextGetHeight(G);
        float* indentFactor = TextGetIndentFactor(G);
        auto* RotMatrix = SceneGetMatrix(G);

#ifndef PURE_OPENGL_ES_2
        RepLabelRenderBackgroundInImmediate(G, I, I->labelV[c].color,
            draw_var, tCenterPt, relativeMode, xn, yn, PmvMatrix, RotMatrix,
      ...
      glPopMatrix();
      {
        // for now, render text twice in immediate mode, some cards
...
#endif
      if (relativeMode & 6) { // label_relative_mode 1 or 2, i.e., screen
                              // stabilized, adjust z
#ifndef PURE_OPENGL_ES_2
        glEnable(GL_FOG);
#endif
...
static void RepLabelGenerateShaderCGO(RepLabel* I, RenderInfo* info)
{
  auto* G = I->G;
  CGO* cgo = CGONew(G);
  auto font_id = SettingGet<int>(
  ...
  for (int c = 0; c < I->N; c++) {
    ...
    if (l) {
      ...
      if (!TextRenderOpenGL(G, info, font_id, st, font_size,
              I->labelV[c].position, (draw_var ? 1 : 0),
              I->labelV[c].getRelativeMode(), true,
              I->shaderCGO)) {
        CGOFree(connectorCGO);
        return;
      }
  ...
  auto* labelCGO = CGOConvertToLabelShader(I->shaderCGO, tmpCGO);
  ...
  CGOFree(I->shaderCGO);
  if (connectorCGO) {
    CGO* tmpCGO = CGOOptimizeConnectors(connectorCGO, 0);
    CGOFree(connectorCGO);
    connectorCGO = tmpCGO;
...
void RepLabelRenderShader(RepLabel* I, RenderInfo* info)
{
  auto pre_use_shaders = info->use_shaders;
  info->use_shaders = true;

  if (!I->shaderCGO) {
    RepLabelGenerateShaderCGO(I, info);
  }
...
void RepLabel::render(RenderInfo* info)
{
  ...
  font_id = SettingCheckFontID(
      G, I->cs->Setting.get(), I->obj->Setting.get(), font_id);

  if (I->shaderCGO && font_size < 0.f) {
    int size;
    if (InvalidateShaderCGOIfTextureNeedsUpdate(
            G, font_size, I->texture_font_size, &size)) {
      CGOFree(I->shaderCGO);
  ...
  if (!use_shader) {
    RepLabelRenderImmediate(I, info);
    return;
  }
  RepLabelRenderShader(I, info);
}
...
Rep* RepLabelNew(CoordSet* cs, int state)
{
  ...
  label_color = SettingGet_i(
      G, cs->Setting.get(), obj->Setting.get(), cSetting_label_color);
  ...
  if (SettingGet_b(
          G, cs->Setting.get(), obj->Setting.get(), cSetting_pickable)) {
  ...
  for (int a = 0; a < cs->NIndex; a++) {
    ...
    if ((ai->visRep & cRepLabelBit) && (ai->label)) {
      ...
      I->N++;
      if ((at_label_color >= 0) || (at_label_color == cColorFront) ||
      ...
      std::copy_n(v0, 3, I->labelV[n].coord);
      {
        ...
        int label_connector = 0, label_bg = 0, label_bg_outline = 0,
            at_con_color = 0, at_label_relative_mode = 0, at_label_z_target,
            label_connector_mode = 0, label_connector_mode_1 = 0,
            label_connector_mode_2 = 0, label_connector_mode_3 = 0,
            label_connector_mode_4 = 0, ray_label_connector_flat = 0;
        ...
        AtomStateGetSetting_i(G, obj, cs, a, ai, cSetting_label_relative_mode,
        ...
        AtomStateGetSetting_b(
            G, obj, cs, a, ai, cSetting_label_bg_outline, &label_bg_outline);
        AtomStateGetSetting_b(
            G, obj, cs, a, ai, cSetting_label_connector, &label_connector);
        AtomStateGetSetting_i(G, obj, cs, a, ai, cSetting_label_connector_mode,
            &label_connector_mode);
        AtomStateGetSetting_i(
            G, obj, cs, a, ai, cSetting_label_z_target, &at_label_z_target);
        ...
        AtomStateGetSetting(
            G, obj, cs, a, ai, cSetting_label_padding, &at_label_padding);
        AtomStateGetSetting_f(G, obj, cs, a, ai, cSetting_label_bg_transparency,
            &at_label_bkgrd_transp);
        AtomStateGetSetting_color(
            G, obj, cs, a, ai, cSetting_label_bg_color, &at_con_color);
        label_bg =
            (at_con_color != -1) &&
            (at_label_bkgrd_transp < 1.f); // if the color is not default and
                                           // transparency is < 1., then draw bg
        drawConnector =
            (label_connector > 0 || label_bg || label_bg_outline > 0) ? 1 : 0;
